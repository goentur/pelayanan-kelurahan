...
export default function Index({gate,tipePenyampaian}:indexProps) {
    const judul = "Jenis Lapor";
    ...
    const getData = async () => {
        setLoading(true);
        try {
            const response = await axios.post(route('master.jenis-lapor.data'), {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const routeName = isEdit 
            ? route('master.jenis-lapor.update', data) 
            : route('master.jenis-lapor.store') as string;
    ...
    const handleHapus = (e: React.FormEvent) => {
        e.preventDefault();
        destroy(route('master.jenis-lapor.destroy', data), {
            preserveScroll: true,
            onSuccess: (e) => {
                setHapus(false);
                alertApp(e);
    ...
    return (
        <AuthenticatedLayout header={judul}>
            ...
            <FormDialog open={form} setOpen={setForm} judul={judul} data={data} setData={setData} errors={errors} formRefs={formRefs} processing={processing} simpanAtauUbah={handleSubmit} tipePenyampaian={tipePenyampaian} />
