...
export default function Index({gate}:indexProps) {
    ...
    const getData = async () => {
        setLoading(true);
        try {
            const response = await axios.post(route('master.jabatan.data'), {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const routeName = isEdit 
            ? route('master.jabatan.update', data) 
    ...
    return (
        <AuthenticatedLayout header={judul}>
            ...
            <FormDialog open={form} setOpen={setForm} judul={judul} data={data} setData={setData} errors={errors} formRefs={formRefs} processing={processing} isEdit={isEdit} simpanAtauUbah={handleSubmit} dataJenis={dataJenis} />
